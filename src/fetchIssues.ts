...
export class FetchIssues {
  ...
  constructor(username: string, password: string, environment: string, featurePath: string, jiraDomain: string) {
    this.jiraUtil = new JiraUtil(username, password, jiraDomain);
    this.fetchTest = new FetchJiraTest(username, password, environment, featurePath, jiraDomain);
  }
    ...
    for (let index = 0; index < keys.length; index++) {
      ...
      switch (result.fields.issuetype.name) {
        ...
        case 'Test Set':
          ...
          (result2.fields[result.fields.issuetype.name === 'Test Plan' ? JiraCustomFields.getProp('testPlanKeys') : JiraCustomFields.getProp('testSetKeys')] || []).forEach((childKey: string) => {
