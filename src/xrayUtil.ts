...
export class XrayUtil {
  ...
  private cucumberUploadUrl = `${this.domain}/rest/raven/1.0/import/execution/cucumber`;
  private linkTestPlanToExecution = `${this.domain}/rest/raven/1.0/api/testplan`;
  ...
  constructor(username: string, password: string, jiraDomain: string) {
    ...
    this.cucumberUploadUrl = `${this.domain}/rest/raven/1.0/import/execution/cucumber`;
  ...
  fetchUsers(test: any): Promise<any> {
    return new Promise((resolve, reject) => {
      if (test && test.fields && test.fields[JiraCustomFields.getProp('preConditions')] && test.fields[JiraCustomFields.getProp('preConditions')].length) {
        this.fetchPreconditions(test.fields[JiraCustomFields.getProp('preConditions')]).then(
          (result) => {
            let preconditions: any = [];
            (result.issues || []).forEach((precondition: any) => {
              if (precondition.fields && precondition.fields.labels && precondition.fields.labels.indexOf('TestUser') >= 0) {
  ...
  async linkTestExecutionToTestPlan(testPlanId: string, testExecutionId: string): Promise<any> {
